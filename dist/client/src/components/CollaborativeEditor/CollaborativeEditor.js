...
const CollaborativeEditor = ({ roomId, initialContent, language = 'typescript' }) => {
    ...
    const updateUserCursor = (data) => {
        setUsers(prev => prev.map(user => user.username === data.username
    ...
    return (<div className="collaborative-editor">
      <div className="editor-container">
        <Editor height="70vh" defaultLanguage={language} value={content} onMount={handleEditorDidMount} onChange={(value) => setContent(value || '')} options={{
        ...
        <div className="users-list">
          ...
          <ul>
            {users.map(user => (<li key={user.username}>
                {user.username}
                {user.cursor && (<span className="cursor-position">
